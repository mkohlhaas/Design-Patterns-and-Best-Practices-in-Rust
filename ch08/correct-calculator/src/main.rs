...
use memento::{
  get_angle_mode, get_calculator_state_type, get_number_base, CalculatorMemento,
  CalculatorStateManager, CalculatorStateType, MementoOriginator, RestoreStateCommand,
  SaveStateCommand,
};
use observer::{
  ...
  fn process_command(&mut self, command: &str) -> Result<Option<f64>, String> {
    ...
    match parts[0] {
      ...
      "mode" => {
        if parts.len() < 2 {
          return Err(
            "Missing mode argument. Use /mode [standard|scientific|programmer]".to_string(),
          );
        }
